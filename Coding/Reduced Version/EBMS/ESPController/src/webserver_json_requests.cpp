...
static constexpr const char *const TAG = "Ebms-webreq";
...
esp_err_t content_handler_coredumpdownloadfile(httpd_req_t *req)
{
  ...
  if (esp_core_dump_image_get(&address, &size) == ESP_OK)
  {
    ...
    if (pt != NULL)
    {
      ...
      httpd_resp_set_type(req, "application/octet-stream");
      httpd_resp_set_hdr(req, "Content-Disposition", "attachment; filename=\"Ebms_coredump.esp32\"");
...
esp_err_t content_handler_downloadfile(httpd_req_t *req)
{
  ...
  if (valid)
  {
    ...
    if ((strncmp(file, "/", sizeof(file)) == 0) || (strncmp(file, "/Ebms/", sizeof(file)) == 0))
    {
