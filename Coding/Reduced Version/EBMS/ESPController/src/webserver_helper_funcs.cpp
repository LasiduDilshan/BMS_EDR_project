...
static constexpr const char *const TAG = "Ebms-webfuncs";

#include "webserver_helper_funcs.h"

char CookieValue[20 + 1];
// EBMS=a&zp!b4okcj$2$Dg*zUC; path=/; HttpOnly; SameSite=Strict
...
void setCookieValue()
{
    ...
    snprintf(cookie, sizeof(cookie), "EBMS=%s; path=/; HttpOnly; SameSite=Strict", CookieValue);
}
...
bool validateXSS(httpd_req_t *req)
{
    ...
    esp_err_t result = httpd_req_get_cookie_val(req, "EBMS", requestcookie, &cookielength);
