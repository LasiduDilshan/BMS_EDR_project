    ...
    $.getJSON("/api/currentmonitor",
        function (data) {
            ...
            if (data.enabled) {
                ...
                if (data.devicetype == 0 || data.devicetype == 2) {
                    //EBMS Current Monitor (internal/external)
    ...
    $.getJSON("/api/modules", { c: cellid },
        function (data) {
            ...
            if (data.settings.Cached == true) {
                ...
                $('#Version').html(data.settings.ver.toString() + ' / <a href="https://github.com/stuartpittaway/EBMSv4ESP32/commit/' + data.settings.code.toString(16) + '" rel="noreferrer" target="_blank">' + data.settings.code.toString(16) + '</a>');
...
$(function () {
    ...
    $("#currentmonitor").click(function () {
        ...
        switchPage("#EbmsCurrentMonitorPage");
    ...
    $("#EbmsCurrentMonitorForm2").unbind('submit').submit(function (e) {
        e.preventDefault();
        currentmonitorSubmitForm(this);
    });
    $("#EbmsCurrentMonitorForm3").unbind('submit').submit(function (e) {
        e.preventDefault();
        currentmonitorSubmitForm(this);
    });
    $("#EbmsCurrentMonitorForm4").unbind('submit').submit(function (e) {
        e.preventDefault();
        currentmonitorSubmitForm(this);
    });
    $("#EbmsCurrentMonitorForm1").unbind('submit').submit(function (e) {
        e.preventDefault();
        currentmonitorSubmitForm(this);
    });
