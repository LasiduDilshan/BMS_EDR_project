...
private:
    ...
    bool SharedChargingDischargingRules(const Ebms_eeprom_settings *mysettings);
...
public:
    ...
    void CalculateChargingMode(const Ebms_eeprom_settings *mysettings, const currentmonitoring_struct *currentMonitor);
    ...
    bool IsStateOfChargeValid(const Ebms_eeprom_settings *mysettings, const currentmonitoring_struct *currentMonitor) const
    {
        return (mysettings->currentMonitoringEnabled &&
                currentMonitor->validReadings &&
                (mysettings->currentMonitoringDevice == CurrentMonitorDevice::EBMS_CURRENT_MON_MODBUS || mysettings->currentMonitoringDevice == CurrentMonitorDevice::EBMS_CURRENT_MON_INTERNAL));
    }
    ...
    bool IsChargeAllowed(const Ebms_eeprom_settings *mysettings);
    bool IsDischargeAllowed(const Ebms_eeprom_settings *mysettings);
    void CalculateDynamicChargeVoltage(const Ebms_eeprom_settings *mysettings, const CellModuleInfo *cellarray);
    void CalculateDynamicChargeCurrent(const Ebms_eeprom_settings *mysettings);
    uint16_t DynamicChargeVoltage() const;
    int16_t DynamicChargeCurrent() const;
    uint16_t StateOfChargeWithRulesApplied(const Ebms_eeprom_settings *mysettings, float realSOC) const;
};
