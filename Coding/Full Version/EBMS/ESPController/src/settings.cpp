...
static constexpr const char *const TAG = "Ebms-set";
...
void DefaultConfiguration(Ebms_eeprom_settings *_myset)
{
    ESP_LOGD(TAG, "Apply default config");

    // Zero all the bytes
    memset(_myset, 0, sizeof(Ebms_eeprom_settings));
    ...
    _myset->currentMonitoringDevice = CurrentMonitorDevice::EBMS_CURRENT_MON_MODBUS;
    ...
    strncpy(_myset->mqtt_topic, "emon/Ebms", sizeof(_myset->mqtt_topic));
...
// Builds up a JSON document which mirrors the parameters inside "Ebms_eeprom_settings"
void GenerateSettingsJSONDocument(DynamicJsonDocument *doc, Ebms_eeprom_settings *settings)
{
    JsonObject root = doc->createNestedObject("Ebms_settings");
    ...
    // wifi["password"] = EBMSSoftAP::Config().wifi_passphrase;
}
...
void JSONToSettings(DynamicJsonDocument &doc, Ebms_eeprom_settings *settings)
{
    ...
    if (!doc.containsKey("Ebms_settings"))
    {
        // Wrong document type - quit...
        return;
    }

    JsonObject root = doc["Ebms_settings"];
