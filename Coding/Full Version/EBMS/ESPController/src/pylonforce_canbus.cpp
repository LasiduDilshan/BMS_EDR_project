...
static constexpr const char *const TAG = "Ebms-pyforce";
...
// 0x7330+Addr / 0x7340+Addr - Transmit the EBMS hostname via two CAN Messages
// Sermatec PDF
...
void pylonforce_message_4210()
{
  ...
  if (mysettings.currentMonitoringEnabled && currentMonitor.validReadings && (mysettings.currentMonitoringDevice == CurrentMonitorDevice::EBMS_CURRENT_MON_MODBUS || mysettings.currentMonitoringDevice == CurrentMonitorDevice::EBMS_CURRENT_MON_INTERNAL))
  {
...
// 0x4260+Addr - Highest / lowest module (EBMS "bank") voltages
...
// 0x4270+Addr - Highest / lowest module (EBMS "bank") temperatures
...
// 0x42e0+Addr / 0x42f0+Addr - Transmit the EBMS hostname via two CAN Messages
...
void pylonforce_handle_rx(twai_message_t *message)
{
  ...
  else if( message->identifier == (0x8210+mysettings.canbus_equipment_addr) )
  {
    ...
    // EBMS relay control is scope of rules, not CAN
    // no reply
