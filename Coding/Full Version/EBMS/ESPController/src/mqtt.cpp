...
static constexpr const char *const TAG = "Ebms-mqtt";
...
void MQTTCurrentMonitoring(const currentmonitoring_struct *currentMonitor)
{
    ...
    if (currentMonitor->validReadings && currentMonitor->timestamp != lastcurrentMonitortimestamp)
    {
        ...
        if (mysettings.currentMonitoringDevice == CurrentMonitorDevice::EBMS_CURRENT_MON_MODBUS || mysettings.currentMonitoringDevice == CurrentMonitorDevice::EBMS_CURRENT_MON_INTERNAL)
        {
