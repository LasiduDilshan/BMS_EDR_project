//Select Environment: env:esp32-devkit(ESPController)
...
static constexpr const char *const TAG = "Ebms";
...
const std::string wificonfigfilename("/Ebms/wifi.json");
...
Rules rules;
Ebms_eeprom_settings mysettings;
                          ...
                          int32_t event_id, void *event_data)
{
  ...
  else if (event_base == IP_EVENT && event_id == IP_EVENT_STA_GOT_IP)
  {
    ...
    ESP_LOGI(TAG, "You can access EBMS interface at http://%s.local or http://%s", hostname.c_str(), ip_string.c_str());
...
void BuildHostname()
{
  ...
  hostname.clear();
  hostname.append("EBMS-").append(buffer);
}
...
void PZEM017_SetDeviceAddress(uint8_t newAddress)
{
  ...
  // support multiple devices on same RS485 bus, but EBMS doesn't....
...
bool CurrentMonitorSetSOC(float newSOC)
{
  if (mysettings.currentMonitoringEnabled == true)
  {
    ...
    if (mysettings.currentMonitoringDevice == CurrentMonitorDevice::EBMS_CURRENT_MON_MODBUS)
    {
      currentMon_SetSOC(newSOC);
      return true;
    }

    if (mysettings.currentMonitoringDevice == CurrentMonitorDevice::EBMS_CURRENT_MON_INTERNAL)
    {
      auto value = (uint16_t)(newSOC * 100);
...
9|ADC Range 0=±163.84 mV, 1=±40.96 mV (only 40.96mV supported by EBMS)|Read only
...
void ProcessEBMSCurrentMonitorRegisterReply(uint8_t length)
{
...
void ProcessEBMSCurrentMonitorInternal()
{
  memset(&currentMonitor.modbus, 0, sizeof(currentmonitor_raw_modbus));
  currentMonitor.validReadings = false;
...
[[noreturn]] void canbus_tx(void *)
{
  for (;;)
  {
    ...
    if (mysettings.canbusprotocol == CanBusProtocolEmulation::CANBUS_PYLONTECH)
    {
      ...
      // https://github.com/PaulSturbo/E-BMS-CAN/blob/main/SEPLOS%20BMS%20CAN%20Protocoll%20V1.0.pdf
...
*/
    ...
    else if (mysettings.canbusprotocol == CanBusProtocolEmulation::CANBUS_VICTRON)
    {
      ...
      // Advertise the EBMS name on CANBUS
      ...
      if (calculatedCRC == crc)
      {
        ...
        if (RS485Error == 0)
        {
          ...
          if (mysettings.currentMonitoringDevice == CurrentMonitorDevice::EBMS_CURRENT_MON_MODBUS)
          {
            ...
            {
              ProcessEBMSCurrentMonitorRegisterReply(length);
...
[[noreturn]] void rs485_tx(void *)
{
  ...
  for (;;)
  {
    ...
    if (mysettings.currentMonitoringEnabled == true)
    {
      ...
      if (mysettings.currentMonitoringDevice == CurrentMonitorDevice::EBMS_CURRENT_MON_INTERNAL)
      {
        if (currentmon_internal.Available())
        {
          ...
          if (hal.GetVSPIMutex())
          {
            ...
            ProcessEBMSCurrentMonitorInternal();
            hal.ReleaseVSPIMutex();
          }
        }
        else
      ...
      if (mysettings.currentMonitoringDevice == CurrentMonitorDevice::EBMS_CURRENT_MON_MODBUS)
      {
        // This is the request we send to EBMS current monitor, it pulls back 38 registers
        // this is all the registers EBMS current monitor has
...
const std::array<log_level_t, 22> log_levels =
    {
        ...
        {.tag = "Ebms", .level = ESP_LOG_DEBUG},
        {.tag = "Ebms-avrisp", .level = ESP_LOG_INFO},
        {.tag = "Ebms-hal", .level = ESP_LOG_INFO},
        {.tag = "Ebms-influxdb", .level = ESP_LOG_INFO},
        {.tag = "Ebms-rx", .level = ESP_LOG_INFO},
        {.tag = "Ebms-tx", .level = ESP_LOG_INFO},
        {.tag = "Ebms-rules", .level = ESP_LOG_INFO},
        {.tag = "Ebms-softap", .level = ESP_LOG_INFO},
        {.tag = "Ebms-tft", .level = ESP_LOG_INFO},
        {.tag = "Ebms-victron", .level = ESP_LOG_INFO},
        {.tag = "Ebms-webfuncs", .level = ESP_LOG_INFO},
        {.tag = "Ebms-webpost", .level = ESP_LOG_INFO},
        {.tag = "Ebms-webreq", .level = ESP_LOG_INFO},
        {.tag = "Ebms-web", .level = ESP_LOG_INFO},
        {.tag = "Ebms-set", .level = ESP_LOG_INFO},
        {.tag = "Ebms-mqtt", .level = ESP_LOG_INFO},
        {.tag = "Ebms-pylon", .level = ESP_LOG_INFO},
        {.tag = "Ebms-pyforce", .level = ESP_LOG_INFO},
