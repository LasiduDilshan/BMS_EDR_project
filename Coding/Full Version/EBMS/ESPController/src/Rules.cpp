...
static constexpr const char *const TAG = "Ebms-rules";
...
bool Rules::SharedChargingDischargingRules(const Ebms_eeprom_settings *mysettings)
{
...
bool Rules::IsChargeAllowed(const Ebms_eeprom_settings *mysettings)
{
...
bool Rules::IsDischargeAllowed(const Ebms_eeprom_settings *mysettings)
{
    if (SharedChargingDischargingRules(mysettings) == false)
        return false;
...
void Rules::CalculateDynamicChargeCurrent(const Ebms_eeprom_settings *mysettings)
{
...
void Rules::CalculateDynamicChargeVoltage(const Ebms_eeprom_settings *mysettings, const CellModuleInfo *cellarray)
{
    if (!mysettings->dynamiccharge || mysettings->canbusprotocol == CanBusProtocolEmulation::CANBUS_DISABLED)
    {
...
uint16_t Rules::StateOfChargeWithRulesApplied(const Ebms_eeprom_settings *mysettings, float realSOC) const
{
    ...
    if (mysettings->socoverride)
    {
        if (value > 90)
        {
            ...
            // even though the battery may not be full, and the EBMS current monitor has not learnt capacity yet.
    ...
    // Limit to 100% maximum, EBMS current monitor can go above 100%, so don't confuse inverter/chargers
    if (value > 100)
    {
        value = 100;
    }
...
/// @param mysettings
/// @param currentMonitor
void Rules::CalculateChargingMode(const Ebms_eeprom_settings *mysettings, const currentmonitoring_struct *currentMonitor)
{
