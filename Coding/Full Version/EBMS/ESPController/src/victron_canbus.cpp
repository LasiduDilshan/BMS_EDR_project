...
static constexpr const char *const TAG = "Ebms-victron";

#include "victron_canbus.h"

// Transmit the EBMS hostname via two CAN Messages
...
void victron_message_355()
{
  ...
  if (_controller_state == ControllerState::Running && mysettings.currentMonitoringEnabled && currentMonitor.validReadings && (mysettings.currentMonitoringDevice == CurrentMonitorDevice::EBMS_CURRENT_MON_MODBUS || mysettings.currentMonitoringDevice == CurrentMonitorDevice::EBMS_CURRENT_MON_INTERNAL))
  {
    data355 data;
    // 0 SOC value un16 1 %
