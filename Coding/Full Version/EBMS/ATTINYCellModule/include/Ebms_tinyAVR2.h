...
#ifndef EBMS_tinyAVR2_H

#define EBMS_tinyAVR2_H
...
/*
This class wraps the hardware pins of EBMS away from the core logic/code
...
public:
  ...
  static inline void DumpLoadOn() { PORTB.OUTSET = PIN1_bm; }  //DumpLoadOn: Sets Pin 1 of Port B to a high state (turns it "on")

  static inline void DumpLoadOff() { PORTB.OUTCLR = PIN1_bm; }  //DumpLoadOff: Sets Pin 1 of Port B to a low state (turns it "off").

  inline static void ReferenceVoltageOn() __attribute__((always_inline))
  {
    // Switch REFERENCE VOLTAGE on
    // Ref voltage ON (PA1): REF_EN
  ...
  /*FlushSerial0: Ensures that all pending serial data is transmitted and clears the buffer.
    DisableSerial0TX: Likely intended to stop further data transmission through the serial port (though the implementation is not provided).*/
  ...
  /*The watchdog timer is used to automatically reset the microcontroller if the software gets stuck 
    or encounters an error, ensuring the system can recover and continue operating correctly.*/
  ...
  static void Sleep()
  {
    ...
    EBMSHAL::EnableStartFrameDetection();
