...
bool PacketProcessor::BypassOverheatCheck()
{
  ...
  return (temp > _config->BypassTemperatureSetPoint || temp > EBMS_MODULE_SafetyTemperatureCutoff);
}
...
#elif (defined(EBMSMODULEVERSION) && (EBMSMODULEVERSION == 430 && defined(SWAPR19R20)))
...
void PacketProcessor::TakeAnAnalogueReading(uint8_t _mode)
{
  ...
  case ADC_CELL_VOLTAGE:
  {
    EBMSHAL::SelectCellVoltageChannel();
    break;
  }
  case ADC_INTERNAL_TEMP:
  {
    EBMSHAL::SelectInternalTemperatureChannel();
    break;
  }
  case ADC_EXTERNAL_TEMP:
  {
    EBMSHAL::SelectExternalTemperatureChannel();
    break;
  }
  default:
  ...
  ADCReading(EBMSHAL::BeginADCReading(_mode));
#else
  EBMSHAL::BeginADCReading(_mode);
#endif
}
...
bool PacketProcessor::processPacket(PacketStruct *buffer)
{
  ...
  case COMMAND::ReadSettings:
  {
    ...
    buffer->moduledata[10] = EBMSMODULEVERSION;
